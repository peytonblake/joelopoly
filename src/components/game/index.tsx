      ...
      <>
        ...
        {turnState == "square" &&
          <>
            {game.board[game.getCurrentPlayer().location] instanceof Property &&
              <>
                ...
                <Button to="/game" onClick={() => setTurnState("property")}>Decide</Button>
              </>
            }
          ...
          {turnState == "property" &&
            ...
            {game.purchasable &&
            <>
              Would {game.players[game.currentPlayer].name} like to buy {tileNames[game.players[game.currentPlayer].location]}?
              <Button to="/game" onClick={() => {setTurnState("endTurn"); game.players[game.currentPlayer].money -= board[game.getCurrentPlayer().location][0]; board[game.getCurrentPlayer().location][11] = game.currentPlayer; game.purchasable = false; game.nextTurn();}}>Buy</Button>
